...
def draw(task_name):
    ...
    leg = fig.legend(handles, labels, loc='lower center', ncol=6, fontsize=FONTSIZE, markerscale=2)
    leg_lines = leg.get_lines()
    for i in range(len(leg_lines)):
        plt.setp(leg_lines[i], linewidth=5.0)

    plt.subplots_adjust(top=1, bottom=0.18, left=0, right=1, hspace=0.13, wspace=0.13)
...
def draw_mnist(task_name):
    ...
    for i in range(len(partition_names)):
        ...
        for agg_index, (agg_code_name, agg_show_name) in enumerate(aggregations):
            ...
            x_axis = [r*record['display_interval']
                        for r in range(record['rounds']+1)]
            axes[i].plot(x_axis, acc_path, '-', color=color, marker=marker, label=agg_show_name, markevery=20, linewidth=4, markersize=20)
