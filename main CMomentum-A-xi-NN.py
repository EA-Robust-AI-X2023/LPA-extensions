...
if 'label' in attack_name:
    if args.aggregation == 'lfighter':
        env = CMomentum_with_LFighter_under_DPA_with_prob(aggregation=aggregation, honest_nodes=honest_nodes, byzantine_nodes=byzantine_nodes, prob=args.prob, attack=attack, step_agg = step_agg,
        ...
        env = CMomentum_under_DPA_with_prob(aggregation=aggregation, honest_nodes=honest_nodes, byzantine_nodes=byzantine_nodes, prob=args.prob, attack=attack, step_agg = step_agg,
                    weight_decay=task.weight_decay, data_package=task.data_package,
                    model=task.model, loss_fn=task.loss_fn, test_fn=task.test_fn,
                    initialize_fn=task.initialize_fn,
                    get_train_iter=task.get_train_iter,
...
print('{:12s} lr={} lr_ctrl={}, alpha={} weight_decay={}'.format(
    '[Optimizer]', super_params['lr'],  env.lr_ctrl.name, super_params['alpha'], task.weight_decay))
